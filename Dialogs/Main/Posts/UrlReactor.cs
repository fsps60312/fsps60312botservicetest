    ...
    public class UrlReactor : MyDialog<IMessageActivity>
    {
        protected override async Task MessageReceivedAsync(IDialogContext context, IAwaitable<IMessageActivity> argument)
        {
            ...
            try
            {
                switch (message.Text)
                {
                    case "https://codingsimplifylife.blogspot.tw/": await context.PostAsync("code風景區！！！歡迎常來逛逛～～～你會發現，寫程式就像欣賞風景一樣快樂哦！"); Main.MarkContextCompleted(message); return;
                    case "https://codingsimplifylife.blogspot.tw/2016/04/c.html": await context.PostAsync("給新手的C++教學！！！號稱網路上對新手最友善的C++教學，歡迎推薦給親朋好友，或提供改善建議哦！"); Main.MarkContextCompleted(message); return;
                    default:
                        {
                            switch (GetPostId(message.Text))
                            {
                                ...
                                case "1995730270697235": await context.Forward(new P1995730270697235(), ResumeAfterAnything, message);done = false; return;
                                default: await context.PostAsync("Oops......這篇文沒有彩蛋哦～試試看別篇吧XD"); Main.MarkContextCompleted(message); return;
                            }
                        }
                }
            }
            finally {if(done) context.Done(message); }
        }
